import React from "react";
import { Card } from "react-bootstrap";
import { motion, AnimatePresence } from "framer-motion";
import { FaPaperPlane, FaSmile } from "react-icons/fa";
import { Form, InputGroup, Button } from "react-bootstrap";
import EmojiPicker from "emoji-picker-react";

/**
 * Trainer Messages UI Component
 * Displays the chat interface from the trainer's perspective
 */
export default function TrainerMessagesUI({
  client,
  messages,
  newMessage,
  setNewMessage,
  showEmojiPicker,
  setShowEmojiPicker,
  onEmojiClick,
  handleSubmit,
  messagesEndRef,
  error,
  trainerId,
}) {
  // Get initials for client avatar
  const getInitials = (name) => {
    if (!name) return "C";
    return name
      .split(" ")
      .map((n) => n[0])
      .join("")
      .toUpperCase()
      .slice(0, 2);
  };

  const clientName = client?.fullName || client?.clientName || client?.name || "Client";

  return (
    <Card
      className="border-0 shadow-lg d-flex flex-column mb-5"
      style={{
        height: "calc(100vh - 160px)",
        borderRadius: "16px",
        overflow: "hidden",
        background: "linear-gradient(135deg, #ffffff 0%, #f8f9fa 100%)",
      }}
    >
      {/* Chat Header - Trainer View */}
      <div
        className="p-3 border-bottom"
        style={{
          background: "linear-gradient(135deg, #00C853 0%, #00B248 100%)",
        }}
      >
        <div className="d-flex align-items-center">
          <div
            className="bg-white rounded-circle d-flex align-items-center justify-content-center me-3"
            style={{
              width: "50px",
              height: "50px",
              color: "#00C853",
              fontWeight: "bold",
              fontSize: "1.2rem",
            }}
          >
            {getInitials(clientName)}
          </div>
          <div className="flex-grow-1">
            <h6 className="text-white mb-0 fw-bold">{clientName}</h6>
            <small className="text-white-50">
              Chatting with {clientName}
            </small>
          </div>
        </div>
      </div>

      {/* Chat Messages */}
      <div
        className="flex-grow-1 overflow-auto p-3"
        style={{
          background:
            "linear-gradient(180deg, #e0f7fa 0%, #ffffff 60%, #e8f5e9 100%)",
        }}
      >
        {messages.length === 0 ? (
          <div className="text-center text-muted mt-5">
            <p>No messages yet. Start the conversation! 💬</p>
            <small>Send a message to your client to get started.</small>
          </div>
        ) : (
          <AnimatePresence>
            {messages.map((msg, idx) => {
              const isSender = msg.senderId === trainerId;
              return (
                <motion.div
                  key={msg.id}
                  initial={{ opacity: 0, y: 10, scale: 0.95 }}
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.95 }}
                  transition={{ duration: 0.3, delay: idx * 0.05 }}
                  className={`d-flex mb-3 ${
                    isSender ? "justify-content-end" : "justify-content-start"
                  }`}
                >
                  <motion.div
                    className="p-3 rounded-4 shadow-sm"
                    style={{
                      maxWidth: "70%",
                      wordBreak: "break-word",
                      background: isSender
                        ? "linear-gradient(135deg, #00C853 0%, #00B248 100%)"
                        : "#ffffff",
                      borderRadius: isSender
                        ? "20px 20px 0 20px"
                        : "20px 20px 20px 0",
                      border: isSender ? "none" : "1px solid rgba(0,0,0,0.1)",
                      color: isSender ? "#ffffff" : "#333",
                    }}
                    whileHover={{ scale: 1.02 }}
                  >
                    <div style={{ fontSize: "0.95rem" }}>{msg.text}</div>
                    <div
                      className={`text-muted ${
                        isSender ? "text-end" : "text-start"
                      }`}
                      style={{
                        fontSize: "0.75rem",
                        marginTop: "4px",
                        color: isSender
                          ? "rgba(255,255,255,0.8)"
                          : "#666",
                      }}
                    >
                      {msg.timestamp?.toDate
                        ? msg.timestamp
                            .toDate()
                            .toLocaleTimeString([], {
                              hour: "2-digit",
                              minute: "2-digit",
                            })
                        : msg.timestamp
                        ? new Date(msg.timestamp).toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })
                        : ""}
                    </div>
                  </motion.div>
                </motion.div>
              );
            })}
          </AnimatePresence>
        )}
        <div ref={messagesEndRef} />
      </div>

      {/* Message Input */}
      <motion.div
        className="bg-light p-3 position-relative border-top"
        style={{
          background: "linear-gradient(135deg, #f8f9fa 0%, #ffffff 100%)",
        }}
        initial={{ y: 50, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ delay: 0.3 }}
      >
        <AnimatePresence>
          {showEmojiPicker && (
            <motion.div
              initial={{ opacity: 0, y: 10, scale: 0.9 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, scale: 0.9 }}
              className="position-absolute"
              style={{
                bottom: "70px",
                left: "10px",
                zIndex: 1000,
                boxShadow: "0 8px 24px rgba(0,0,0,0.15)",
                borderRadius: "16px",
                overflow: "hidden",
              }}
            >
              <EmojiPicker onEmojiClick={onEmojiClick} />
            </motion.div>
          )}
        </AnimatePresence>

        <Form onSubmit={handleSubmit}>
          <InputGroup className="align-items-center">
            <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
              <Button
                variant="light"
                className="rounded-circle me-2 border-0 shadow-sm d-flex align-items-center justify-content-center"
                style={{
                  width: "45px",
                  height: "45px",
                  backgroundColor: "#00C853",
                }}
                onClick={() => setShowEmojiPicker((prev) => !prev)}
                type="button"
              >
                <FaSmile size={20} color="white" />
              </Button>
            </motion.div>

            <Form.Control
              placeholder="Type a message..."
              value={newMessage}
              onChange={(e) => setNewMessage(e.target.value)}
              className="rounded-pill border-0 shadow-sm px-4 py-2"
              style={{
                backgroundColor: "#ffffff",
                fontSize: "1rem",
                minHeight: "45px",
              }}
            />

            <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
              <Button
                type="submit"
                variant="primary"
                className="rounded-circle ms-2 border-0 shadow-sm d-flex align-items-center justify-content-center"
                style={{
                  width: "45px",
                  height: "45px",
                  backgroundColor: "#00C853",
                }}
                disabled={!newMessage.trim()}
              >
                <FaPaperPlane />
              </Button>
            </motion.div>
          </InputGroup>
        </Form>
      </motion.div>
    </Card>
  );
}

